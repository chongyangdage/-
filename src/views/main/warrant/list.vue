 ...
 <el-form-item label-width="120px" label="真实姓名" prop="empname">
  ...
  <span slot="footer" class="dialog-footer">
    ...
    <div class="tree">
      ...
      <div>
        <mul @click_node='click_node' @ok="choose" :isCheckbox="true" :data="data_tree"></mul>
      </div>
...
import mul from "@/views/layout/components/Tree/register";
import { mul_api } from "@/api/criteria";
import { index_gl } from "@/api/people";
export default {
  components: {
    mul1,
    mul,
  ...
  methods: {
    ...
    click_node(e){
      this.id=e
      let data={"criteria":{"_expr":[{"orgidlist": this.id,"_op":"="},{"empstates":2,"_op":"="}]},page: { length: this.page_size, isCount: true, begin: this.page*10 }}
      index_gl(data).then(res=>{
    ...
    change() {
      this.changeUser = this.new_orgin;
      let data={employee:{orgidlist:this.id,empid:this.changeUser}}
      mul_api(data).then(res=>{
